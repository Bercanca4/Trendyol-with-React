...
function App() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('https://raw.githubusercontent.com/Bercanca4/api/main/api.json');
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('https://raw.githubusercontent.com/Bercanca4/api/main/cardsData.json');
        const data = await response.json();
        setCardsData(data);
      } catch (error) {
        console.error('Error fetching data:', error);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('https://raw.githubusercontent.com/Bercanca4/api/main/productData.json');
        const data = await response.json();
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('https://raw.githubusercontent.com/Bercanca4/api/main/interestedCategoryData.json');
        const data = await response.json();
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('https://raw.githubusercontent.com/Bercanca4/api/main/informationData.json');
        const data = await response.json();
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('https://raw.githubusercontent.com/Bercanca4/api/main/popularSideData.json');
        const data = await response.json();
   ...
   const dataLists = [
    ...
    {  data: "İndirim Kuponlarım", link: 'https://www.trendyol.com/giris?cb=/Hesabim/IndirimKuponlari' },
    {  data: "Trendyol'da Satış yap", link: 'https://partner.trendyol.com/onboarding/satici-formu' },
    {  data: "Hakkımızda", link: 'https://www.trendyol.com/aboutus' },
    {  data: "Yardım & Destek", link: 'https://www.trendyol.com/yardim/sorular' },
  ...
  <div className="w-[1200px] mx-auto ">
   ...
   {/* Top Side - Bu alanda mini list arrayden geliyor */}
    ...
    {CategoryData.map((item, index) => (
        <Categories  key={index} name={item.name} statu={item.statu === 'true' ? <p> Yeni</p> : ( <p style={{ backgroundColor: 'white' }}>{item.statu}</p> ) } />
      ))} 
    </div>
  </div>
  ...
  <Slider {...settings}>
    ...
    <Brands  key={index} Brandsname={item.Brandsname} logoUrl={item.logoUrl} link={item.link} />
      ))}  
   </Slider>
    </div>
      ...
      <Cards  key={index} id={item.id} content={item.content} logoUrl={item.logoUrl} link={item.link} />
      ))} 
      </div>
      ...
      <div className="flex items-cente justify-between m-4">
        ...
        <div>
          <label className="text-[22px] font-semibold opacity-70">Çok Satan Ürünler</label>
        </div>
        
        <div >
           <a href="https://www.trendyol.com/sr?wg=1&sst=BEST_SELLER">
          <label className="text-[14px] hover:cursor-pointer  opacity-70">Tüm ürünler!</label>
          </a>
        </div>
     ...
     <div className="flex items-center  gap-x-4 ml-3">
      ...
      <ProductList  key={index} id={item.id} logoUrl={item.logoUrl} price={item.price} brandname={item.brandname} title={item.title} />
      ))} 
    </div>
      ...
      <div className="flex items-cente justify-between m-4">
        ...
        <div>
          <label className="text-[22px] font-semibold opacity-70">Avantajlı Ürünler</label>
        </div>
     ...
     <div className="flex items-center  gap-x-4 ml-3">
      ...
      <div>
        <label className="text-[20px] font-semibold">Bunlar da İlginizi Çekebilir</label>
      </div>
      ...
      <InterestedCategory  key={index} name={item.name} link={item.link}/>
      ))} 
      </div>
      ...
      <Information  key={index} title={item.title} descreption={item.descreption} />
      ))} 
      </div>
      </div>
    </div>  
    ...
    <div className="w-[1200px] h-[503px] my-4  mx-auto  items-center justify-center flex">
      ...
      <div className=" h-full flex-1  p-8 ">
      <div><label className="text-[20px] p-4">Popüler Sayfalar</label></div>
